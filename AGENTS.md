...
Lint the diff, not the tree. `prek run --all-files` hands every hook the whole repo instead of your diff, so `lint-frontend`'s `files:` filter still matches every `.js`/`.vue`/`.scss`/`.css` in the tree — even on a branch that changed none: 64 `node` workers, 24.8 GB RSS, enough to OOM-kill a 31 GB host. CI does that on a disposable runner (`.github/workflows/pre-commit.yml`); it is not a reason to run it locally. The scoped forms print `(no files to check)Skipped` when your diff has no frontend files — that is a pass, not a gap.
