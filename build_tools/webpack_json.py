...
def validate_modules(build_list):
    ...
    for module in build_list:
        if module not in valid_module_names:
                logging.info('{} did not initialize.'.format(module))
        else:
                logging.info('Successfully initialized {}'.format(module))
...
def main():
    ...
    if args.output_file:
        logging.info("Writing webpack_json output to {}".format(args.output_file))
        with open(args.output_file, 'w') as f:
            json.dump(result, f)
    else:
        logging.info("No output file argument; writing webpack_json output to stdout.")
