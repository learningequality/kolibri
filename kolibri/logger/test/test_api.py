...
class ExamAttemptLogAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        [ExamAttemptLog.objects.create(item="d4623921a2ef5ddaa39048c0f7a6fe06", examlog=self.examlog,
            user=self.user1, content_id=uuid.uuid4().hex, channel_id=uuid.uuid4().hex,
            start_timestamp=str(datetime.datetime.now()), end_timestamp=str(datetime.datetime.now()), correct=0) for _ in range(3)]
