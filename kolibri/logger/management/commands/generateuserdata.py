...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        # Specify a set number of users to generate per class
    ...
    def handle(self, *args, **options):  # noqa: max-complexity=16
        ...
        # Choose the first channel in the cache - this is the only unpredictable part,
        # as it depends on the channels loaded, also it means that generated data is restricted to content that
        ...
        for facility in facilities:
            ...
            # Create objects for each classroom that will be generated
            classes = [
                Classroom(
            ...
            for i, classroom in enumerate(classes):
                # Save the classroom object now so that facility users can be added to them
                classroom.save()
                ...
                for base_data in facility_user_data[i*options['users']: (i+1)*options['users']]:
                    ...
                    # If this user has been created before, don't recreate them
                    # Note that this may add users to multiple classes, but this is fine
