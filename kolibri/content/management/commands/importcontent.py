...
class Command(AsyncCommand):
    ...
    def download_content(self, channel_id):
        self._transfer(DOWNLOAD_METHOD, channel_id, tracker=None)
    ...
    def _transfer(self, method, channel_id, path=None, tracker=None):  # noqa: max-complexity=16
        ...
        if method == DOWNLOAD_METHOD:
            with self.start_progress(total=total_bytes_to_transfer) as overall_progress_update:
                self._transfer_all_files(files_to_download, overall_progress_update, 
                    file_checksums_to_annotate, method, path, downloaded_files)

        elif method == COPY_METHOD:
            self._transfer_all_files(files_to_download, tracker, file_checksums_to_annotate, 
                method, path, downloaded_files)


    def _transfer_all_files(self, files_to_download, overall_progress_update, 
        file_checksums_to_annotate, method, path, downloaded_files):
    ...
    def handle_async(self, *args, **options):
        if options['command'] == 'network':
            ...
            self.copy_content(options["channel_id"], options["directory"], options["tracker"])
        else:
