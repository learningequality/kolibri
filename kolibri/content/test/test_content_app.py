...
from kolibri.content.constants import content_kinds
...
class ContentNodeTestCase(TestCase):
    ...
    def test_get_ancestor_topics(self):
        ...
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["root", "c2"])
        actual_output = api.get_ancestor_topics(channel_id=self.the_channel_id, content=p)
        self.assertEqual(set(expected_output), set(actual_output))

    def test_immediate_children(self):
        p = content.ContentNode.objects.using(self.the_channel_id).get(title="root")
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["c1", "c2"])
    ...
    def test_leaves(self):
        ...
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["c2c1", "c2c2", "c2c3"])
    ...
    def test_get_all_prerequisites(self):
        ...
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["root"])
        ...
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["c1"])
    ...
    def test_get_all_related(self):
        ...
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["c2"])
    ...
    def test_children_of_kind(self):
        p = content.ContentNode.objects.using(self.the_channel_id).get(title="root")
        expected_output = content.ContentNode.objects.using(self.the_channel_id).filter(title__in=["c2", "c2c2", "c2c3"])
...
class ContentNodeAPITestCase(APITestCase):
    ...
    def test_contentnode_recommendations(self):
        c1_id = str(content.ContentNode.objects.using(self.the_channel_id).get(title="root").content_id)
        response = self.client.get(self._reverse_channel_url("contentnode-recommendations", {'content_id': c1_id}))
