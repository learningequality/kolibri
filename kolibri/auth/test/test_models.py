...
class CollectionRoleMembershipDeletionTestCase(TestCase):
    ...
    def test_remove_coach(self):
        self.assertTrue(self.classroom_coach.has_role_for_collection(role_kinds.FACILITY_COACH, self.lg))
        self.assertTrue(self.classroom_coach.has_role_for_collection(role_kinds.FACILITY_COACH, self.cr))
        self.assertFalse(self.classroom_coach.has_role_for_collection(role_kinds.FACILITY_COACH, self.facility))
        self.assertFalse(self.classroom_coach.has_role_for_collection(role_kinds.ADMIN, self.lg))
        self.assertTrue(self.classroom_coach.has_role_for_user(role_kinds.FACILITY_COACH, self.learner))
        self.assertFalse(self.classroom_coach.has_role_for_user(role_kinds.FACILITY_COACH, self.facility_admin))
        self.assertFalse(self.classroom_coach.has_role_for_user(role_kinds.ADMIN, self.learner))
        self.assertEqual(Role.objects.filter(user=self.classroom_coach, kind=role_kinds.FACILITY_COACH, collection=self.cr).count(), 1)

        self.cr.remove_coach(self.classroom_coach)

        self.assertFalse(self.classroom_coach.has_role_for_collection(role_kinds.FACILITY_COACH, self.lg))
        self.assertFalse(self.classroom_coach.has_role_for_collection(role_kinds.FACILITY_COACH, self.cr))
        self.assertFalse(self.classroom_coach.has_role_for_collection(role_kinds.FACILITY_COACH, self.facility))
        self.assertFalse(self.classroom_coach.has_role_for_collection(role_kinds.ADMIN, self.lg))
        self.assertFalse(self.classroom_coach.has_role_for_user(role_kinds.FACILITY_COACH, self.learner))
        self.assertFalse(self.classroom_coach.has_role_for_user(role_kinds.FACILITY_COACH, self.facility_admin))
        self.assertFalse(self.classroom_coach.has_role_for_user(role_kinds.ADMIN, self.learner))
        self.assertEqual(Role.objects.filter(user=self.classroom_coach, kind=role_kinds.FACILITY_COACH, collection=self.cr).count(), 0)
    ...
    def test_remove_admin(self):
        ...
        self.assertFalse(self.facility_admin.has_role_for_collection(role_kinds.FACILITY_COACH, self.lg))
        ...
        self.assertFalse(self.facility_admin.has_role_for_user(role_kinds.FACILITY_COACH, self.learner))
...
class CollectionsTestCase(TestCase):
    ...
    def test_add_and_remove_coach(self):
        ...
        self.assertEqual(Role.objects.filter(user=user, kind=role_kinds.FACILITY_COACH, collection=self.classroom).count(), 1)
        self.assertEqual(Role.objects.filter(user=user, kind=role_kinds.FACILITY_COACH, collection=self.facility).count(), 1)
        self.classroom.remove_coach(user)
        self.facility.remove_coach(user)
        self.assertEqual(Role.objects.filter(user=user, kind=role_kinds.FACILITY_COACH, collection=self.classroom).count(), 0)
        self.assertEqual(Role.objects.filter(user=user, kind=role_kinds.FACILITY_COACH, collection=self.facility).count(), 0)
    ...
    def test_add_coaches(self):
        ...
        self.assertEqual(Role.objects.filter(kind=role_kinds.FACILITY_COACH, collection=self.classroom).count(), 2)
        self.assertEqual(Role.objects.filter(kind=role_kinds.FACILITY_COACH, collection=self.facility).count(), 2)
