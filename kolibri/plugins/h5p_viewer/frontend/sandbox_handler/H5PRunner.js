...
export default class H5PRunner {
  ...
  /*
   * H5P's state and the learner's user data both live on the shim, which is what
   * persists them and what receives updates from Kolibri. Read them through rather
   * than copying the references: an update replaces the object, and a copy would
   * leave us reading state nobody writes to and writing state nobody reads.
   */
  get data() {
    return this.shim.data;
  }

  get userData() {
    return this.shim.userData;
  }

  async init(iframe, filepath) {
    ...
    await this.recurseDependencies('h5p.json', true);
    ...
    // The frame can still be on the blank document it was created with, which is
    // also complete, so only a completed document at another URL is the host page.
    const hostPage = this.iframe.contentDocument;
    const hostPageLoaded =
      hostPage && hostPage.readyState === 'complete' && hostPage.URL !== 'about:blank';
    if (!hostPageLoaded) {
      await new Promise(resolve => this.iframe.addEventListener('load', resolve, { once: true }));
    }
    await this.initH5P();
  }
   ...
   */
  async initH5P() {
    if (!this.iframe.contentWindow?.H5P) {
      throw new Error('The H5P host page loaded without H5P');
    }
    // H5PIntegration is installed by H5PShim from the content page's <head>,
    // before any of the H5P host page's own scripts run.
    this.shimH5P(this.iframe.contentWindow);
    await this.scriptLoader(this.cssURL, true);
    await this.scriptLoader(this.javascriptURL);
    // Start H5P
    ...
    // The error rejects init, which reports it to the user to avoid confusion and
  ...
  /*
   * The object H5P reads to configure itself. Every field is a getter, so this
   * can be handed out before the H5P file has been processed - the values
   * resolve whenever H5P actually reads them. Built once, so H5PIntegration
   * keeps a stable identity across reads as it did when set as a plain value.
   */
  buildIntegration() {
    if (this.integrationShim) {
      return this.integrationShim;
    }
    const self = this;
