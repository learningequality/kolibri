...
import { Statement } from './xAPISchema';
...
function actorsEqual(actor1, actor2) {
  ...
  if (actor1.openid && actor2.openid) {
    return actor1.openid === actor2.openid;
  }
  if (actor1.account && actor2.account) {
    return (
      actor1.account.homePage === actor2.account.homePage &&
      actor1.account.name === actor2.account.name
    );
  }
  return false;
}
...
export default class xAPIShim extends SandboxShim {
  ...
  getStatements({
    ...
    // A related_* filter is an OR across the places its value may appear, but the
    // spec ANDs the filters with each other, as the non-related paths do via `match`.
    const andFns = [];
    ...
    if (agent) {
      if (!related_agents) {
        ...
        andFns.push(
          overSome([
            s => s.actor && actorsEqual(s.actor, agent),
            s => s.object && actorsEqual(s.object, agent),
            s =>
              s.object &&
              s.object.objectType === OBJECT_TYPES.SUBSTATEMENT &&
              s.object.actor &&
              actorsEqual(s.object.actor, agent),
            ...
            s => s.authority && actorsEqual(s.authority, agent),
          ]),
        );
      }
    }
        ...
        andFns.push(
          overSome([
            ...
            s =>
              s.object &&
              s.object.objectType === OBJECT_TYPES.SUBSTATEMENT &&
              isMatch(s.object, activityMatch),
            s =>
              s.context &&
    ...
    // sendStatement stores a statement Statement.clean rejected, unnormalised.
    statements = statements.filter(s => !s.error);
    for (const fn of andFns) {
      statements = statements.filter(fn);
    }
    if (since) {
  ...
  sendValue(nameSpace, id, value) {
    return new Promise((resolve, reject) => {
      try {
        ...
        return reject('Can only store JSON serializable values');
      }
      if (!id) {
        return reject('Must specify an id');
      }
  ...
  /*
   * A simple interface for interacting with xAPI, exposed on the content
   * window as `xAPI`. Only H5P content gets this shim, so only H5P content
   * can use it to directly manage xAPI data.
   */
  __setXAPIInterface() {
    const self = this;
    ...
    class Shim {
       ...
       */
      async sendStatement(statement, compress = false) {
        ...
        // A statement Statement.clean rejected is stored unnormalised, and may be
        // missing the object this block dereferences - compress it only when it is
        // whole, so the reject still reaches storeStatements.
        if (compress && statement.object) {
