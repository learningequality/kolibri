...
const fakeUser = {
  roles: [{ id: 'role_1', collection: 'facility_1' }, { id: 'role_3', collection: 'class_3' }],
};
...
describe('removeCoachRoleAction', () => {
  ...
  it('successfully removes the correct Role on server and client', done => {
    ...
    removeCoachRoleAction(storeMock, {
      userId: 'user_1',
      classId: 'class_3',
    }).then(() => {
  ...
  it('handles when fetching User fails', done => {
    ...
    removeCoachRoleAction(storeMock, {
      ...
      assert.deepEqual(storeMock.dispatch.getCall(2).args, ['CORE_SET_ERROR', '"fetch error"']);
      done();
    });
  });
  ...
  it('handles when deleting Role fails', done => {
    ...
    removeCoachRoleAction(storeMock, {
      ...
      assert.deepEqual(storeMock.dispatch.getCall(2).args, ['CORE_SET_ERROR', '"delete error"']);
      done();
    });
  });
});
