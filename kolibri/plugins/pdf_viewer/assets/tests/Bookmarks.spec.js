...
import Bookmarks from '../src/views/Bookmarks/index';
import BookmarkItem from '../src/views/Bookmarks/item';
...
describe('Pdf Bookmarks', () => {
  ...
  it('should render dropdown-icon for bookmarks with children', () => {
    const wrapper = makeWrapper();
    outline.forEach((bookmark) => {
      const bookmarkItem = withWrapperArray(wrapper.findAllComponents(BookmarkItem)).hasText(bookmark.title).at(0)
  ...
  it('should render children bookmarks when click on dropdown', async () => {
    ...
    for (const bookmark of outline) {
      if (!bookmark.items.length) continue;
        ...
        expect(
          withWrapperArray(bookmarkItem.findAllComponents(BookmarkItem)).hasText(bookmark.items[0].title)
        ).toHaveLength(0);
