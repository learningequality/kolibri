...
describe('useCourseNotificationPolling', () => {
  ...
  describe('classroom-wide mode (courseSessionId is null)', () => {
    ...
    describe('scoped to the current classroom', () => {
      ...
      it('resets when classId changes, so a stale timestamp does not prevent polling', async () => {
        const notifications = reactive([]);
        const store = makeStore({ timestamp: '2024-01-01T10:00:00Z', notifications });
        const callback = jest.fn();
        const classId = ref('classroom-a');

        useCourseNotificationPolling(store, null, callback, classId);

        classId.value = 'classroom-b';
        store.getters['coachNotifications/maxNotificationTimestamp'] = '2024-01-01T09:00:00Z';
        await nextTick();
        ...
        store.getters['coachNotifications/maxNotificationTimestamp'] = '2024-01-01T09:30:00Z';
