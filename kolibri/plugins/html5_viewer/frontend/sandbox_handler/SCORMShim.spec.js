...
describe('SCORM shim', () => {
  ...
  let mediator;
  beforeEach(() => {
    // The shim traces every SCORM API call at debug level, which jest-fail-on-console
    // would otherwise treat as a failure.
    jest.spyOn(console, 'debug').mockImplementation(() => {});
    mediator = new Mediator(window);
    scorm = new SCORMShim(mediator);
  });
  afterEach(() => {
    mediator.destroy();
  ...
  describe('shim management and instance methods', () => {
    ...
    describe('initialize method', () => {
      it('should set API property on object and its parent', () => {
        const obj = { parent: {} };
        scorm.initialize(obj);
        expect(obj.API).toEqual(scorm.shim);
        expect(obj.parent.API).toEqual(scorm.shim);
      });
    });
    describe('LMSInitialize method', () => {
      ...
      it('should report the restored state, carrying its progress to main', () => {
        const state = { cmi: { core: { lesson_status: 'passed' } } };
        scorm.setData(state);
        ...
        expect(sent).toHaveBeenCalledWith({
          nameSpace: 'sandbox',
          event: scorm.events.SHIMSTATEUPDATE,
          data: { shim: 'SCORM', state, progress: 1 },
        });
      });
    });
    ...
    describe('getProgress method', () => {
      it('should report no progress until the content says something', () => {
        expect(scorm.getProgress()).toBeNull();
      });
      it('should scale a raw score between its reported min and max', () => {
        shim.LMSSetValue('cmi.core.score.min', 2);
        shim.LMSSetValue('cmi.core.score.max', 12);
        shim.LMSSetValue('cmi.core.score.raw', 7);
        expect(scorm.getProgress()).toEqual(0.5);
      });
      it('should treat a raw score as a percentage when min and max are unset', () => {
        shim.LMSSetValue('cmi.core.score.raw', 25);
        expect(scorm.getProgress()).toEqual(0.25);
      });
      it('should fall back to lesson_status when no score is reported', () => {
