...
describe('fileExtractors', () => {
  describe('video extractor', () => {
    ...
    it('extracts files from video with source children', () => {
      ...
      // getAttribute('src') returns the raw attribute, so the URL is the value set
    ...
    it('derives extension from the URL suffix for a direct src', () => {
      const video = createElement('video', { src: 'http://example.com/clip.webm' });
      const files = extractors.video(video);

      expect(files[0].extension).toBe('webm');
    });

    it('falls back to the URL suffix when the source type is not in the map', () => {
      const source = createElement('source', { src: 'clip.ogv', type: 'video/ogg' });
      const video = createElement('video', {}, [source]);

      const files = extractors.video(video);

      expect(files[0].extension).toBe('ogv');
    });

    it('derives extension from the URL suffix when source has no type', () => {
      const source = createElement('source', { src: 'clip.webm' });
      const video = createElement('video', {}, [source]);
    ...
    it('falls back to the container default when source has no type and no URL suffix', () => {
      const source = createElement('source', { src: 'stream' });
      const video = createElement('video', {}, [source]);
  ...
  describe('audio extractor', () => {
    ...
    it('falls back to the container default when source has no type and no URL suffix', () => {
      ...
      expect(files[0].extension).toBe('mp3');
    });
  });
