...
<script>
  ...
  import vue, { computed, ref, watch, onMounted, onBeforeUnmount, getCurrentInstance } from 'vue';
  ...
  export default {
    ...
    setup(props, context) {
      ...
      const videoSources = computed(() => {
        return files.value.filter(file => videoPresets.has(file.preset));
      });
      ...
      const transcriptStyle = computed(() => {
        if (!transcriptWrap.value) {
          return {
            borderLeft: `1px solid ${instance.$themeTokens.fineLine}`,
          };
        }
        return {
      ...
      const throttledResizePlayer = throttle(resizePlayer, 300);
      ...
      onMounted(() => {
        initPlayer(getPlayerConfig);
        window.addEventListener('resize', throttledResizePlayer);
      });

      onBeforeUnmount(() => {
        window.removeEventListener('resize', throttledResizePlayer);
      });
