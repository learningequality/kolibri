...
function createMockPlayer({ duration = 100, currentTime = 0, seeking = false } = {}) {
  let time = currentTime;
  return ref({
    duration: () => duration,
    currentTime: () => time,
    seeking: () => seeking,
    // Test helper: simulate playback advancing to a new position.
    _setCurrentTime: t => {
      time = t;
    },
  });
...
describe('useMediaProgress', () => {
  describe('recordProgress', () => {
    it('reports the absolute durationBasedProgress and ignores elapsed time when forced', () => {
      ...
      player.value._setCurrentTime(50);
      updateTime();
    ...
    it('emits addProgress as the fraction of duration newly elapsed since the last record', () => {
      ...
      player.value._setCurrentTime(30);
      updateTime();
      player.value._setCurrentTime(50);
      updateTime();
  ...
  describe('updateTime', () => {
    ...
    it('does not re-record until another 5 seconds have elapsed', () => {
      ...
      player.value._setCurrentTime(9);
      updateTime();
      expect(emit).not.toHaveBeenCalled();

      player.value._setCurrentTime(11);
      updateTime();
  ...
  describe('handleSeek', () => {
    it('flushes sub-threshold progress before a seek and rebaselines after it', () => {
      ...
      player.value._setCurrentTime(3);
      updateTime();
      expect(emit).not.toHaveBeenCalled();

      player.value._setCurrentTime(80);
      handleSeek();
      expect(emit).toHaveBeenCalledWith('addProgress', 0.03);

      emit.mockClear();
      player.value._setCurrentTime(85);
      updateTime();
      expect(emit).toHaveBeenCalledWith('addProgress', 0.05);
    });
  });
