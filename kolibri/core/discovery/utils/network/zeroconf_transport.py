...
class ZeroconfNetworkDiscovery(EventBusHost):
    ...
    def _addresses_changed(self):
        ...
        if self.interfaces is not InterfaceChoice.All:
            return False
    ...
    def register(self, instance):
        ...
        try:
            ...
            # the backend never broadcast, so no `stop_listening` will follow
            self.stop_listening()
            raise
    ...
    def _is_own_service(self, name):
        # read once: `stop_listening` can null it between two reads
        own = self.instance
        return own is not None and own.is_broadcasting and own.service_info.name == name
    ...
    def _build_instance(self, service_info):
        ...
        # the no-op callbacks drop what we build. Read once, as the stop can
        # also land between two reads
        own = self.instance
        is_self = own is not None and instance.zeroconf_id == own.zeroconf_id
