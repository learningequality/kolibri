...
class ZeroconfNetworkDiscoveryTestCase(SimpleTestCase):
    ...
    def test_register_rename_gives_up(self):
        ...
        # nothing else will release the Zeroconf a failed register opened
    ...
    def test_added_event_for_our_own_zeroconf_id_marks_is_self(self):
        ...
        self._handle(f"{MOCK_ID}.{SERVICE_TYPE}", ServiceStateChange.Added)
        (instance,), _ = self.on_add.call_args
    ...
    def _stop_on_read(self, obj, attr, value):
        """Lands a `stop_listening` on the next read of `obj.attr`."""

        def stop_then_read():
            self.transport.stop_listening()
            return value

        setattr(type(obj), attr, mock.PropertyMock(side_effect=stop_then_read))

    def test_own_service_check_racing_a_stop_is_dropped_not_raised(self):
        # the stop can land between the guard's reads of our own instance
        self._listen()
        self._stop_on_read(self.instance, "is_broadcasting", True)
        self._handle(PEER_SERVICE_NAME, ServiceStateChange.Added)
        self.on_add.assert_not_called()

    def test_instance_built_racing_a_stop_is_not_our_own(self):
        self._listen()
        peer = mock.Mock(spec_set=KolibriInstance)("peer")
        self._stop_on_read(peer, "zeroconf_id", "peer")
        with mock.patch(
            ZEROCONF_MODULE + "KolibriInstance.from_service_info", return_value=peer
        ):
            self.transport._build_instance(build_service_info("peer"))
        peer.set_broadcasting.assert_called_once_with(mock.ANY, is_self=False)
