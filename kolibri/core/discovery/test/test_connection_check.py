...
class TestCheckConnection(TestCase):
    ...
    def test_check_with_no_previous_information(self):
        self.assertFalse(check_connection(FAKE_KOLIBRI_ONLINE))
        self.assertFalse(check_connection(FAKE_KOLIBRI_OFFLINE))
        self.assertEqual(check_connection(REAL_KOLIBRI_ONLINE), VALID_DEVICE_INFO)
        self.assertFalse(check_connection(REAL_KOLIBRI_OFFLINE))
