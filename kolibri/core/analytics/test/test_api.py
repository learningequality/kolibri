...
class PingbackNotificationDismissedTestCase(APITestCase):
    ...
    def test_create_notification(self):
        response = self.client.post(reverse('kolibri:core:pingbacknotificationdismissed-list'),
                                    {'user': self.admin.id, 'notification': self.notification.id}, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(PingbackNotificationDismissed.objects.count(), 1)

    def test_filter_notification(self):
        PingbackNotificationDismissed.objects.create(user=self.admin, notification=self.notification)
        response = self.client.get(reverse('kolibri:core:pingbacknotificationdismissed-list'),
                                   {'user': self.admin.id, 'notification': self.notification.id}, format='json')
        self.assertEqual(len(response.data), PingbackNotificationDismissed.objects.count())

    def test_filter_non_existent_notification(self):
        response = self.client.get(reverse('kolibri:core:pingbacknotificationdismissed-list'),
                                   {'user': self.admin.id, 'notification': 'non-existent-id'}, format='json')
        self.assertEqual(len(response.data), 0)

    def test_can_not_create_other_user_notification(self):
        response = self.client.post(reverse('kolibri:core:pingbacknotificationdismissed-list'),
                                    {'user': self.user.id, 'notification': self.notification.id}, format='json')
