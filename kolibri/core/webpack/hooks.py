...
class WebpackBundleHook(hooks.KolibriHook):
    ...
    def bundle(self):
        ...
        for f in self._stats_file_content["files"]:
            filename = f["name"]
            if not getattr(settings, "DEVELOPER_MODE", False) and any(
                regex.match(filename) for regex in IGNORE_PATTERNS
            ):
                continue
    ...
    def get_stats(self, unique_id=None):
        ...
        try:
            return json.loads(
                ...
                .joinpath("build")
                .joinpath(f"{unique_id}_stats.json")
                .read_text()
            )
        except OSError as e:
            raise WebpackError(f"Error accessing stats file '{unique_id}': {e}") from e
