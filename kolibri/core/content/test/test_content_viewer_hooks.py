...
class SandboxedContentViewerHookTestCase(TestCase):
    def setUp(self):
        # define_hook makes hook classes singletons, so the stats cache set by
        # one test is still on the instance the next one gets.
        self.hook = SandboxHook()
        if hasattr(self.hook, "_cached_sandbox_handler_stats"):
            del self.hook._cached_sandbox_handler_stats
    ...
    def _patch_stats_file(self, *contents):
        ...
        if len(contents) == 1:
            stats_file.read_text.return_value = contents[0]
        else:
            stats_file.read_text.side_effect = list(contents)
        return (
            patch("kolibri.core.webpack.hooks.files", return_value=stats_file),
            stats_file,
        )

    def test_stats_file_read_once_outside_developer_mode(self):
        patcher, stats_file = self._patch_stats_file('{"status": "done", "chunks": {}}')
    ...
    def test_handler_stats_wait_for_an_in_progress_compile(self):
        patcher, stats_file = self._patch_stats_file(
            '{"status": "compile"}',
            '{"status": "done", "chunks": {"handler": ["chunk"]}}',
        )
        with patcher, self.settings(DEVELOPER_MODE=True):
            stats = self.hook._get_sandbox_handler_stats()

        self.assertEqual(stats["chunks"], {"handler": ["chunk"]})

    def test_handler_stats_raise_on_a_failed_compile(self):
        patcher, _ = self._patch_stats_file('{"status": "error", "chunks": {}}')
        with patcher, self.settings(DEVELOPER_MODE=True):
            with self.assertRaises(WebpackError):
                self.hook._get_sandbox_handler_stats()
