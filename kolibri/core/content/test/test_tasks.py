...
class ValidateContentTaskTestCase(TestCase):
    ...
    def test_missing_channel_id(self):
        with self.assertRaises(serializers.ValidationError):
            ChannelValidator(
                data={
                    "task": "kolibri.core.content.tasks.remotecontentimport",
                    "channel_name": "test",
                }
    ...
    def test_invalid_channel_id(self):
        with self.assertRaises(serializers.ValidationError):
            ChannelValidator(
                data={
                    "task": "kolibri.core.content.tasks.remotechannelimport",
                    "channel_id": "test",
                    "channel_name": "test",
                }
            ).is_valid(raise_exception=True)
    ...
    def test_wrong_drive_id(self):
        with self.assertRaises(serializers.ValidationError):
            LocalChannelImportValidator(
                data={
                    "task": "kolibri.core.content.tasks.localchannelimport",
                    "channel_id": uuid.uuid4().hex,
                    "channel_name": "test",
                    "drive_id": "test",
                }
