...
from .paths import get_content_dir_path
...
def set_leaf_node_importability_from_local_file_importability(channel_id):
    ...
    file_statement = select([LocalFileTable.c.importable]).where(
        FileTable.c.local_file_id == LocalFileTable.c.id,
    ).limit(1)

    logger.info('Setting importability of File objects based on LocalFile importability')

    connection.execute(FileTable.update().values(importable=file_statement).execution_options(autocommit=True))
    ...
    logger.info('Setting importability of non-topic ContentNode objects based on File importability')
    ...
    ).values(importable=exists(contentnode_statement)).execution_options(autocommit=True))
...
def recurse_importability_up_tree(channel_id):
    ...
    logger.info('Setting importability of ContentNode objects with children for {levels} levels'.format(levels=node_depth))
    ...
    for level in range(node_depth, 0, -1):
        ...
        logger.info('Setting importability of ContentNode objects with children for level {level}'.format(level=level))
        # Only modify topic importability here
        connection.execute(ContentNodeTable.update().where(
            and_(
                ContentNodeTable.c.level == level - 1,
                ContentNodeTable.c.channel_id == channel_id,
                ContentNodeTable.c.kind == content_kinds.TOPIC)).values(importable=exists(importable_nodes)))
