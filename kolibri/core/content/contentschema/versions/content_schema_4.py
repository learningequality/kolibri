# coding: utf-8
from sqlalchemy import Boolean, CHAR, Column, Float, ForeignKey, Index, Integer, String, Text
...
class ContentAssessmentmetadata(Base):
    ...
    contentnode_id = Column(ForeignKey('content_contentnode.id'), nullable=False, index=True)
...
class ContentContentnodeHasPrerequisite(Base):
    ...
    __table_args__ = (
        Index('content_contentnode_has_prerequisite_from_contentnode_id_to_contentnode_id_c9e1d527_uniq', 'from_contentnode_id', 'to_contentnode_id', unique=True),
    )

    id = Column(Integer, primary_key=True)
    from_contentnode_id = Column(ForeignKey('content_contentnode.id'), nullable=False, index=True)
    to_contentnode_id = Column(ForeignKey('content_contentnode.id'), nullable=False, index=True)

    from_contentnode = relationship('ContentContentnode', primaryjoin='ContentContentnodeHasPrerequisite.from_contentnode_id == ContentContentnode.id')
    to_contentnode = relationship('ContentContentnode', primaryjoin='ContentContentnodeHasPrerequisite.to_contentnode_id == ContentContentnode.id')


class ContentContentnodeRelated(Base):
    __tablename__ = 'content_contentnode_related'
    __table_args__ = (
        Index('content_contentnode_related_from_contentnode_id_to_contentnode_id_fc2ed20c_uniq', 'from_contentnode_id', 'to_contentnode_id', unique=True),
    )
    ...
    from_contentnode = relationship('ContentContentnode', primaryjoin='ContentContentnodeRelated.from_contentnode_id == ContentContentnode.id')
    to_contentnode = relationship('ContentContentnode', primaryjoin='ContentContentnodeRelated.to_contentnode_id == ContentContentnode.id')


class ContentContentnodeTags(Base):
    __tablename__ = 'content_contentnode_tags'
    __table_args__ = (
        Index('content_contentnode_tags_contentnode_id_contenttag_id_64a4ac15_uniq', 'contentnode_id', 'contenttag_id', unique=True),
    )

    id = Column(Integer, primary_key=True)
    contentnode_id = Column(ForeignKey('content_contentnode.id'), nullable=False, index=True)
    contenttag_id = Column(ForeignKey('content_contenttag.id'), nullable=False, index=True)
...
class ContentFile(Base):
    ...
    contentnode_id = Column(ForeignKey('content_contentnode.id'), nullable=False, index=True)
    lang_id = Column(ForeignKey('content_language.id'), index=True)
    local_file_id = Column(ForeignKey('content_localfile.id'), nullable=False, index=True)
