...
class DeviceProvisionTestCase(APITestCase):
    ...
    def test_cannot_post_if_provisioned(self):
        ...
        response = self.client.post(reverse('kolibri:deviceprovision'), data, format="json")
    ...
    def test_superuser_created(self):
        ...
        self.client.post(reverse('kolibri:deviceprovision'), data, format="json")
...
class DevicePermissionsTestCase(APITestCase):
    ...
    def test_superuser_delete_own_permissions(self):
        response = self.client.delete(reverse('kolibri:devicepermissions-detail', kwargs={'pk': self.superuser.devicepermissions.pk}), format="json")
        self.assertEqual(response.status_code, 403)

    def test_superuser_update_own_permissions(self):
        response = self.client.patch(reverse('kolibri:devicepermissions-detail',
    ...
    def test_posix_freespace(self):
        if not sys.platform.startswith('win'):
            with mock.patch('kolibri.utils.system.os.statvfs') as os_statvfs_mock:
                ...
                response = self.client.get(reverse("kolibri:freespace"), {'path': 'test'})
    ...
    def test_win_freespace_fail(self):
        if sys.platform.startswith('win'):
            ...
            with mock.patch.dict('sys.modules', ctypes=ctypes_mock):
                ...
                try:
                    self.client.get(reverse('kolibri:freespace'), {'path': 'test'})
                except OSError:
...
class DeviceInfoTestCase(APITestCase):
    ...
    def test_has_version(self):
        response = self.client.get(reverse('kolibri:deviceinfo'), format="json")
