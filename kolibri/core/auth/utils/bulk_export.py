...
def bulk_export_users(
    facility_id, locale=None, use_storage=False, output_file=None, overwrite=True
):
    ...
    locale = settings.LANGUAGE_CODE if not locale else locale
    translation.activate(locale)

    if facility_id:
        default_facility = Facility.objects.get(pk=facility_id)
    else:
        default_facility = Facility.get_default_facility()
    if not default_facility:
        raise CommandError(MESSAGES[NO_FACILITY])

    filename = CSV_EXPORT_FILENAMES["user"].format(
        default_facility.name, default_facility.id[:4]
    )
    ...
    if job:
        job.update_progress(0, total_rows)
    ...
    try:
        for i, row in enumerate(
            ...
            if job:
                job.update_progress(i, total_rows)
    except (ValueError, IOError) as e:
        raise CommandError(MESSAGES[FILE_WRITE_ERROR].format(e))

    if job:
        job.extra_metadata["overall_error"] = []
        job.extra_metadata["users"] = total_rows
        job.extra_metadata["filename"] = filename
        job.save_meta()
    else:
        logger.info("Created csv file {} with {} lines".format(filename, total_rows))
