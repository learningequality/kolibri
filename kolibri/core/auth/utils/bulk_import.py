...
class BulkUserImportManager(JobProgressMixin):
    def __init__(
        self,
        filepath,
        facility_id=None,
    ...
    def build_users_objects(self, users):
        ...
        for user, user_row in users.items():
            self.progress_update(progress)
            values = self.get_field_values(user_row)
            if values["uuid"] in existing_users:
                error = self._build_existing_user(
                    user, user_row, values, keeping_users, update_users
                )
            else:
                error = self._build_new_user(user, user_row, values, new_users)
            if error:
                per_line_errors.append(error)

        return (new_users, update_users, keeping_users, per_line_errors)

    def _user_error(self, user, user_row, message, field, value):
        return {
            "row": user_row["position"],
            "username": user,
            "message": message,
            "field": field,
            "value": value,
    ...
    def _build_existing_user(self, user, user_row, values, keeping_users, update_users):
        ...
        if user_obj.username != user:
            ...
            if existing_user:
                return self._user_error(
                    user, user_row, MESSAGES[DUPLICATED_USERNAME], "USERNAME", user
                )
        if self.compare_fields(user_obj, values):
            update_users.append(user_obj)
        return None

    def _build_new_user(self, user, user_row, values, new_users):
        ...
        if values["uuid"] != "":
            return self._user_error(
                user,
                user_row,
                MESSAGES[NON_EXISTENT_UUID].format(user),
                "PASSWORD",
                "*",
            )
        if not values["password"]:
            return self._user_error(
                user, user_row, MESSAGES[REQUIRED_PASSWORD], "PASSWORD", "*"
            )
    ...
    def get_facility(self):
        if self.facility_id:
            default_facility = Facility.objects.get(pk=self.facility_id)
        else:
            default_facility = Facility.get_default_facility()
        if not default_facility:
            raise BulkUserImportError(str(MESSAGES[NO_FACILITY]))
    ...
    def get_delete(self, keeping_users, update_classes):
        if not self.delete:
            return ([], [])
        ...
        if self.userid:
            users_not_to_delete.append(self.userid)
    ...
    def exit_if_error(self):
        if self.overall_error:
            ...
            if self.job:
                ...
                self.job.save_meta()
            raise BulkUserImportError(f"File errors: {self.overall_error}")
        return
    ...
    def run(self):
        self.default_facility = self.get_facility()

        storage_filepath = None
        local_filepath = None

        if self.use_storage:
            storage_filepath = self.filepath
        else:
            local_filepath = os.path.join(os.getcwd(), self.filepath)
        ...
        with self.start_progress(total=100) as self.progress_update:
            ...
            users_to_delete, classes_to_clear = self.get_delete(
                keeping_users, db_update_classes
            )
