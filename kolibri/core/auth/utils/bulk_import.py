...
def csv_values_validation(reader, header_translation, facility):
    per_line_errors = []
    ...
    for classroom in coach_classrooms:
        ...
        if normalized_name in normalized_learner_classroooms:
            ...
            if classroom != real_name:
                validator.coach_classrooms[real_name] = validator.coach_classrooms.pop(
                    classroom
                )
    return (
        per_line_errors,
...
class _BulkUserImportRunner:
    """
    Holds the per-run state for a single bulk-import operation (parsed facility,
    accumulated errors, progress, etc.), decoupled from Django's management
    Command machinery so it can be driven directly by a task or a CLI command.
    """
    ...
    def progress_update(self, increment):
        # Clamp to 100 to match the legacy JobProgressMixin behavior.
        self._progress = min(self._progress + increment, 100)
        if self.job:
            self.job.update_progress(self._progress, 100)
    ...
    def build_classes_objects(self, classes):
        """
        Returns an updated classes list, using the case insensitive
    ...
    def get_facility(self, facility):
        if facility:
            default_facility = Facility.objects.get(pk=facility)
        else:
    ...
    def get_delete(self, delete, userid, keeping_users, update_classes):
        if not delete:
            return ([], [])
        ...
        if userid:
            users_not_to_delete.append(userid)
    ...
    def save_job_metadata(
        self, per_line_errors, classes_report, users_report, filename
    ):
    ...
    def exit_if_error(self):
        if self.overall_error:
            ...
            if self.job:
                self.save_job_metadata(0, classes_report, users_report, "")
    ...
    def output_messages(
        ...
        if self.job:
            self.save_job_metadata(
                per_line_errors, classes_report, users_report, ntpath.basename(filepath)
            )
        else:
    ...
    def run(
        ...
        self.default_facility = self.get_facility(facility)
        ...
        self.locale = settings.LANGUAGE_CODE if not locale else locale
        translation.activate(self.locale)
        ...
        if self.job:
            self.job.update_progress(0, 100)
        ...
        try:
            with open_csv_for_reading(
                ...
                per_line_errors, classes, users, roles = csv_values_validation(
        ...
        users_to_delete, classes_to_clear = self.get_delete(
            delete, userid, keeping_users, db_update_classes
        )
