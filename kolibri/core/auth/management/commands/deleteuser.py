...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('username', action='store', type=str, help='Username of user to delete')
        parser.add_argument('--facility', action='store', type=str, help='Facility ID that user is associated with')

    def handle(self, *args, **options):
        try:
            if options['facility']:
                user = FacilityUser.objects.get(username=options['username'], facility_id=options['facility'])
            else:
            ...
            raise CommandError(
                ('There is more than one user on this device with the username `{username}`. '
                 'Please specify the facility ID for this user.'.format(username=options['username']))
            )
