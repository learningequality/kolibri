...
class FacilityTasksAPITestCase(APITestCase):
    ...
    def test_startdataportalsync(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kolibri:core:task-list"),
            {
                ...
                "task": "kolibri.core.auth.tasks.dataportalsync",
            },
            format="json",
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_startpeerfacilityimport(
        ...
        req_data = dict(
            facility=self.facility.id,
            task="kolibri.core.auth.tasks.peerfacilityimport",
            username="testuser",
            password="testpass",
            device_id=device.id,
        )
    ...
    def test_startpeerfacilitysync(
        ...
        req_data = dict(
            facility=self.facility.id,
            task="kolibri.core.auth.tasks.peerfacilitysync",
            username="testuser",
            password="testpass",
            device_id=device.id,
        )
    ...
    def test_startdeletefacility(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kolibri:core:task-list"),
            dict(facility=facility2.id, task="kolibri.core.auth.tasks.deletefacility"),
            format="json",
        )
        self.assertEqual(response.status_code, 200)
        self.assertJobResponse(fake_job_data, response)
    ...
    def test_startdeletefacility__not_superuser(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kolibri:core:task-list"),
            dict(facility=facility1.id, task="kolibri.core.auth.tasks.deletefacility"),
            format="json",
        )
    ...
    def test_startdeletefacility__facility_member(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kolibri:core:task-list"),
            dict(
                facility=self.facility.id, task="kolibri.core.auth.tasks.deletefacility"
            ),
            format="json",
        )
...
class FacilityTaskHelperTestCase(TestCase):
    ...
    def test_validate_no_facility__validation_error(self):
        with self.assertRaises(serializers.ValidationError):
            SyncJobValidator(
                data={"task": "kolibri.core.auth.tasks.peerfacilitysync"}
            ).is_valid(raise_exception=True)

    def test_validate_empty_facility_id__validation_error__empty(self):
        with self.assertRaises(serializers.ValidationError):
            SyncJobValidator(
                data=dict(task="kolibri.core.auth.tasks.peerfacilitysync", facility="")
            ).is_valid(raise_exception=True)
    ...
    def test_validated_data(self):
        ...
        validator = SyncJobValidator(
            data=dict(
                task="kolibri.core.auth.tasks.peerfacilitysync", facility=facility_id
            )
        )
