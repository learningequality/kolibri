<template>
  <div v-show="!$isPrint" :style="{ backgroundColor: $themeTokens.appBar }">
    ...
    <header>
      ...
      <UiToolbar :title="title" type="clear" textColor="white" class="app-bar" :style="{ height: topBarHeight + 'px' }"
        :raised="false" :removeBrandDivider="true">
        <template #icon>
          <KIconButton icon="menu" :color="$themeTokens.textInverted" :ariaLabel="$tr('openNav')"
            @click="$emit('toggleSideNav')" />
        </template>

        <template #brand>
          <img v-if="themeConfig.appBar.topLogo" :src="themeConfig.appBar.topLogo.src"
            :alt="themeConfig.appBar.topLogo.alt" :style="themeConfig.appBar.topLogo.style" class="brand-logo">
        </template>
        ...
        <template #actions>
          <div aria-live="polite" :style="{
            ...
            <span v-if="isLearner">
              <KIconButton ref="pointsButton" icon="pointsActive" :ariaLabel="$tr('pointsAriaLabel')" />
              <div v-if="pointsDisplayed" class="points-popover" :style="{
