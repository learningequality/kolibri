...
export function objectPropValidator(options) {
  ...
  var _getType = function(variable) {
    ...
    return match
      ...
      : rawType.length > 0
      ? rawType
      : typeof variable == 'function' || typeof variable.name != 'undefined'
      ? variable.name
      : '';
  };
  ...
  var _isPureObject = function(variable) {
    return (
      typeof variable === 'object' && typeof Object != 'undefined' && variable === Object(variable)
    );
  };
  ...
  var validatorLogger = function(msg, dataType, option, config, dataKey) {
    ...
    if (!(_isSet(config.logLevel) && config.logLevel === null)) {
      if (_isSet(config.logLevel) && config.logLevel == 'throw') {
        ...
        _isUndefined(config.logLevel) ||
        (_isSet(config.logLevel) && !console.hasOwnProperty(config.logLevel))
      ) {
  ...
  var validator = function(data, option, config, dataKey) {
    ...
    if (
      _isUndefined(data) &&
      _isSet(option.required) &&
      option.required &&
      _isSet(option.type) &&
      !_isEmpty(option.type)
    ) {
    ...
    if (
      _isSet(data) &&
      _isSet(option.validator) &&
      _isFunction(option.validator) &&
      !option.validator(data)
    ) {
  ...
  var config =
    _isSet(args[1]) && _isObject(args[1])
      ? args[1]
      : {
          enabled: true,
          logLevel: 'error',
