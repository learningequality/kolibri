...
For content that requires sandboxed execution (HTML5 apps, H5P, Bloom, etc.), use ``SandboxedContentViewerHook`` instead. This hook also requires a sandbox handler bundle in ``buildConfig.js`` with ``sandbox_handler: true``, whose ``bundle_id`` is ``sandbox_handler`` unless the hook sets ``sandbox_handler_id``.
