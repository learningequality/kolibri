...
Resource methods resolve with the data, not the response.

.. code-block:: javascript

  // corresponds to /api/content/channel/<id>
  const channel = await ChannelResource.retrieve(id);

  // corresponds to /api/content/channel/?available=true
  const channels = await ChannelResource.list({ available: true });

.. seealso::
  :doc:`/frontend_architecture/resource_layer` for the full method surface and the composables that call it from a component.
