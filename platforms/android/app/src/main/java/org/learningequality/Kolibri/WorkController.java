...
public class WorkController {
  ...
  protected static volatile WorkController mInstance;
  ...
  protected void dispatch(Message message, int attempts) {
    ...
    synchronized (isConnected) {
      ...
      if (!isConnected.get()) {
        ...
        context.bindService(
            new Intent(context, WorkControllerService.class), connection, Context.BIND_AUTO_CREATE);
      }
    }
    ...
    connected.thenApply(
        (msg) -> {
          try {
            ...
            synchronized (isConnected) {
              isConnected.set(false);
              WorkController.this.messenger = null;
            }
            if (attempts < 3) {
