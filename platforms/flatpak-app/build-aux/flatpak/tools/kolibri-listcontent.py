...
class OutputWriter_Plain(OutputWriter):
    def write(self, output):
        for content_list in self.visible_content_lists:
            click.echo(
                f"{click.style(content_list.channel_name, bold=True)} v{content_list.channel_version} ({content_list.channel_id})",
                file=output,
            )
            click.secho(
                f"{content_list.pick_nodes_count} content nodes - {content_list.required_bytes} bytes",
                dim=True,
                file=output,
            )
...
class OutputWriter_INI(OutputWriter):
    def write(self, output):
        ...
        output.write(f"# {local_now()}\n")
        output.write("\n")

        total_required_bytes = sum(
            cl.required_bytes for cl in self.filtered_content_lists
        )

        output.write("[kolibri]\n")
        output.write(f"total_required_bytes = {total_required_bytes}\n")
        output.write("install_channels =\n")
        for content_list in self.visible_content_lists:
            output.write(
                f"  # {content_list.channel_name} [{content_list.pick_nodes_count}]\n"
            )
            output.write(f"  {content_list.channel_id}\n")
        output.write("\n")

        for content_list in self.filtered_content_lists:
            output.write(f"[kolibri-{content_list.channel_id}]\n")
            output.write(f"version = {content_list.channel_version}\n")
            output.write(f"required_bytes = {content_list.required_bytes}\n")
