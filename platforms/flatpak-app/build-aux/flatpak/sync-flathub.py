...
checkout and pin the kolibri-gnome module to a Kolibri release tag.
...
KOLIBRI_REPO_URL = "https://github.com/learningequality/kolibri.git"


def pin_kolibri_gnome(data: dict, ref: str, commit: str, subdir: str) -> None:
    """Build the kolibri-gnome module from subdir of the Kolibri repo at ref/commit.
    ...
    for module in data["modules"]:
        ...
        for source in git_sources:
            source.update(url=KOLIBRI_REPO_URL, tag=ref, commit=commit)
        return
...
def main(flathub_dir: str, ref: str) -> None:
    ...
    # Kolibri commit, and running from the flathub checkout would
    # otherwise silently pin a commit from the wrong repository.
    commit = subprocess.check_output(
        ["git", "rev-parse", ref], text=True, cwd=HERE
    ).strip()
    subdir = subprocess.check_output(
        ["git", "rev-parse", "--show-prefix"], text=True, cwd=HERE.parents[1]
    ).rstrip("/\n")
    ...
    pin_kolibri_gnome(data, ref, commit, subdir)
