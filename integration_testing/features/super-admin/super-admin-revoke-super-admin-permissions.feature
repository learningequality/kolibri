Feature: Super admin removes all device permissions from another Super admin
  ...
  Scenario: Super admin can see what happens when they revoke super admin permissions to another user
    ...
    Then I see the confirmation snackbar *Changes saved*
      And I see *User type* has changed to their previous facility role
