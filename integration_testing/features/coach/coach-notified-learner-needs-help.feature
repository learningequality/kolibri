...
# Prepare two browsers, or two windows/tabs of the same browser, one of them being incognito/private mode, in order to sign into one as a learner user, and as a coach in the other
  ...
  Scenario: Learner starts the exercise   
    ...
    Then as a coach <coach> in another window I see the *'<learner>' has started '<exercise>'* notification in the *Coach > Class home > Class activity* block
  ...
  Scenario: Coach gets notification that learner needs help with an exercise
    When as learner <learner> I intentionally make 3 incorrect answers to a question
    Then within 5 seconds as a coach <coach> in another window I see the *'<learner>' needs help with '<exercise>'* notification in the *Coach > Class home > Class activity* block
