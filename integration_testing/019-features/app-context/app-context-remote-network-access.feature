Feature: Access Kolibri app server remotely
	...
	Scenario: Sign in directly after entering the username
		...
		When I try to access the Kolibri server app from a browser on an external device # You don't need to be in an external device: use an incongnito window, or any other browser where the cookie for the App user is not set
