...
function extract$trs(scriptString, namespace, filePath) {
  ...
  if(filePath.includes('.vue')) {
    ...
    componentName = fileName === "index.vue" ?
    ...
    pre: node => {
      function _getObjectifiedValue(nodePropertyValue) {
          ...
          if(nodePropertyValue.type !== "ObjectExpression") {
            ...
            const message = messageNode.value.type === "TemplateLiteral" ?
              get(messageNode, 'value.quasis[0].value.raw') :
              get(messageNode, 'value.value', null);

            const context = contextNode.value.type === "TemplateLiteral" ?
      ...
      if(isCreateTranslator(node)) {
        ...
        if(node.arguments[1].type === "Identifier") { 
          ...
          if(!messageNodeProperties) {
            logging.error(`Could not find variable called ${varToken} where strings for ${namespace} should be defined.`);
        ...
        messageNodeProperties.forEach($trProperty => {
          const newMessages = { [`${componentName}.${_getPropertyKey($trProperty)}`]: _getObjectifiedValue($trProperty.value) };
...
try {
  ...
  logging.error(`Invalid value for --project flag: Please ensure that you've included the \`--project=<PROJECT>\` flag and that the <PROJECT> you entered is one of ${SUPPORTED_PROJECTS.map(e => `\`${e}\``).join(' or ')}.`);
  process.exit(1);
}
...
if (process.argv.indexOf('--dry-run') === -1) {
  ...
  let count = 0;
  Object.keys(extractedMessages).forEach(ns => count += Object.keys(extractedMessages[ns]).length)
