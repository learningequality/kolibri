...
export default class BloomRunner {
  ...
  init(iframe, filepath, loaded, errored) {
    // console.log('Bloom Runner called', iframe, filepath, loaded, errored);
    ...
    // const x = this.zip.allFiles()
    // console.log(x, "Files")
    // // Recurse all the package dependencies
    // return this.recurseDependencies('meta.json', true).then(() => {
    ...
    // return this.setDependencies().then(() => {
    return this.processFiles().then(() => {
      ...
      return this.processContent().then(() => {
    ...
    return this.scriptLoader(this.cssURL, true).then(() => {
      return this.scriptLoader(this.javascriptURL).then(() => {
        try {
          ...
          // console.log(this.contentUrl, 'contnet Url');
          this.loaded();
          this.iframe.src = this.contentUrl;
          // setTimeout(() => {
   ...
   */
  setDependencies() {
    // return this.zip.filesFromExtension();
  ...
  processJsDependencies() {
    const concatenatedJS = this.jsDependencies.reduce((wholeJS, dependency) => {
      return `${wholeJS}${dependency}\n\n`;
    }, '');
  ...
  processCssDependencies() {
    const concatenatedCSS = this.cssDependencies.reduce((wholeCSS, dependency) => {
      return `${wholeCSS}${dependency}\n\n`;
    }, '');
  ...
  //   const htmlContent = decoder.decode(this.contentfile.obj);
  //   const parser = new DOMParser();
  //   const doc = parser.parseFromString(htmlContent, 'text/html');

  //   console.log('Testing procerss contnet');
  ...
  //             const fileUrl = 'hue.png';
  //             const updatedStyle = style.replace(
  //               new RegExp(`url\\(["']?${filePath}["']?\\)`, 'g'),
  //               `url('${fileUrl}')`,
  //             );
  ...
  //   this.contentUrl = URL.createObjectURL(
  ...
  processContent() {
    return new Promise((resolve, reject) => {
      try {
        ...
        // console.log('Testing process content');
          ...
          if (style) {
            ...
            if (urls) {
              urls.forEach(url => {
                ...
                if (urlMatch) {
                  const filePath = urlMatch[1];
                  const stylePromise = this.zip.file(filePath).then(file => {
                    // console.log(file, filePath);
                    let fileUrl = file._url;
                    if (!fileUrl) {
                      fileUrl = URL.createObjectURL(new Blob([file.obj]));
                    }
                    const updatedStyle = style.replace(
                      new RegExp(`url\\(["']?${filePath}["']?\\)`, 'g'),
                      `url('${fileUrl}')`,
        ...
        // Wait for all style promises to complete
  ...
  processFiles() {
    return Promise.all([
      this.zip.filesFromExtension('.css').then(cssFiles => (this.cssDependencies = cssFiles)),
      this.zip.filesFromExtension('.js').then(jsFiles => (this.jsDependencies = jsFiles)),
      this.zip.filesFromExtension('.htm').then(htmFile => {
        this.contentfile = htmFile[0];
        this.contentUrl = URL.createObjectURL(
          new Blob([this.contentfile.obj], { type: 'text/html' }),
        );
      }),
    ]);
  }
