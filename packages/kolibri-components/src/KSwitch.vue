<template>
  ...
  <label class="k-switch" :class="classes">
    <div class="k-switch__input-wrapper">
      <input
        ref="input"
      ...
      <div class="k-switch__thumb">
        <div class="k-switch__focus-ring"></div>
      </div>

      <div class="k-switch__track"></div>
    </div>

    <div v-if="label || $slots.default" class="k-switch__label-text">
      <slot>{{ label }}</slot>
    </div>
  </label>
...
<style lang="scss">
  ...
  .k-switch {
    ...
    &.is-disabled {
      ...
      .k-switch__input-wrapper,
      .k-switch__label-text {
  ...
  .k-switch__input {
    position: absolute;
    opacity: 0;
    body[modality='keyboard'] &:focus + .k-switch__thumb {
      .k-switch__focus-ring {
        opacity: 1;
        transform: scale(1);
      }
    }
